...
def get_client(
    ...
    """Get an authenticated Garmin client using file-based tokens."""
...
def get_activity_exercise_sets(client: Garmin, activity_id: int) -> dict:
    ...
    time.sleep(1.0)
    response = client.client.request("GET", "connectapi", f"/activity-service/activity/{activity_id}/exerciseSets")
        ...
        for ex in exercises:
            ...
            if normal:
                ...
                if has_distance or (has_duration and not has_weight):
                    ...
                    lines.append(f"- {name}: {len(normal)} {n_label}; {top_weight:.1f}kg x {top_reps}")
            elif warmup:
