...
from .mapper import lookup_exercise
...
def build_exercise_sets_payload(
    ...
    for si in all_sets:
        ...
        cat_id, sub_id, _ = lookup_exercise(ex)
...
def attempt_merge(client, hevy_workout: dict, database, overlap_threshold: float = 0.70, max_drift_minutes: int = 20) -> MergeResult:
    ...
    try:
        ...
        if not desc.endswith("- synced by hevy-sync"):
            desc += "\n- synced by hevy-sync"
        desc = "Exercises synced from Hevy by hevy-sync\n\n" + desc
